...
\section{Introduction} \label{sec:introduction}
	...
	\subsection{Problem}
		...
		The ability to save their designs as prefabs could support the sharing of prefabs within the community, allowing others to benefit from the sharing of knowledge regarding workload requirements.
	...
	\subsection{Research Questions and Approach}
		...
		\begin{itemize}
			%TODO: flesh this section out a lot more, better merge it with what used to be Approach
			% - split approach per RQ
			...
			It must also support a variety of operations to add, remove and modify data stored within it to facilitate user working with it.
			...
			We can use prototyping to become more familiar with these new technologies, analysing their strenghts and potential shortcomings.
			We also may use prototyping to determine what is and isn't useful in our initial data structure design, and iteratively improve our design with these findings.
			...
			We can also use the prototype to determine if any additional changes need to be made to the existing codebase to support our extensions, and whether any of these modifications may adversely impact functionality.
			...
			To do this, we should assess the impact that the addition of prefabs have on performing certain tasks in \opendc{}, and determine what the benefits of prefabs are in concrete terms.
			...
			In Section~\ref{sec:evaluation}, we detail our experimental setup for subjectively testing the improvement that prefabs bring to \opendc{}.
	...
	\subsection{Thesis Outline}
		...
		We assess both the implemented data structure, as well as the implemented methods of interacting with the data structure, using a different testing methodology for each.
...
\section{Background} \label{sec:background}
	...
	\subsection{Formats for Specifying Datacenter Topology Modules}
		\thoughts{I am honestly considering cutting this and putting some of it somewhere else (in the design, perhaps). What do you think?}
...
\section{Designing Representation of Data Center Hardware} \label{sec:design}
	...
	\subsection{Model for Datacenter Representation}
		...
		Our model, however, offers more detail, storing a wider range of the characteristics of the hardware components within.
		...
		\subsubsection{Central Processing Unit(s)}
			...
			While commercially available servers, such as Lenovo's ThinkSystem SR950 \footnotetext{\url{https://lenovopress.com/lp0647-thinksystem-sr950-server-xeon-sp-gen-1}}, may contain up to 8 CPUs in a single chassis, we choose not to place an artificial limit on the CPU count.
			...
			Additionally, this provides the flexibility required to represent multi-chassis servers, such as HPE's Superdome Flex, that scale up to 32 sockets while functioning as one physical server to the operating system (and thus the workload).
		...
		\subsubsection{Random Access Memory}
			...
			We use 1 as the number of interfaces, in order to calculate the speed per module.
			The combined memory bandwidth of a system increases linearly as we add modules.
			We choose again not to limit the number of memory modules that can be added to a system.
			While our findings suggest that Lenovo's aforementioned ThinkSystem SR950 has the most memory modules of any single-node system, with 96 in total, we don't want to artificially constrain researchers and datacenter operators from experimenting with theoretical systems with much higher memory density.

		\subsubsection{Graphics Processing Unit(s)}
			When representing Graphics Processing Units (GPUs), we choose to represent what we believe to be the most important properties of a GPU.
			These are its name, the number of GPU cores it contains, the clock speed of the GPU cores, and the power consumption.
	...
	\subsection{Designing Ways to Create and Interact with Prefabs}
		...
		Such functionality serves as the basis for the behaviour highlighted in \textbf{FR2}, where users would be able to then create new topologies from prefabs, or add prefabs into their existing topologies.
		...
		Lastly, the inclusion of some \opendc{} prefabs detailed in \textbf{FR5} is important to provide a basis for new prefabs.
		Such a provision would also provide a reference for how prefabs can look to new users.
...
\label{tab:3}
\thoughts{I am considering removing operators we do not use from this table, but i feel it undermines the mention of KLM a little: i would rather give a better understanding of KLM operators than have a slightly smaller table}
\end{table}
...
\newpage
\appendix
\thoughts{keeping stuff i might need here for later}
\appendixpage
\addappheadtotoc
\begin{table}[]
			\centering
